Page({
  ...
  goto: function () {
    //筛选出数量大于0的商品
    let selectedGoods = this.data.cart.filter(item => item.count > 0);
    wx.navigateTo({
      url: '../settlement/settlement?goods=' + JSON.stringify(selectedGoods),
    })
  },
    //自动生成订单号
    gotoSettlement: function () {
      ...
      const orderId = 'LY' + timestamp;
      // 筛选出数量大于0的商品
      let selectedGoods = this.data.cart.filter(item => item.count > 0);
      wx.navigateTo({
        url: '../settlement/settlement?orderId=' + orderId + '&goods=' + JSON.stringify(selectedGoods),
      })
    }
})